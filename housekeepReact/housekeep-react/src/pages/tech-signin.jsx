...
import { technicianLogin } from '../features/profile.slice';

const validationSchema = yup.object({
  email: yup.string('').required('Required'),
  password: yup.string('').required('Required'),
});
export const TechSignIn = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const resp = await dispatch(technicianLogin(formik.values));
    console.log('resp', resp.payload);

    if (resp.payload === 'Login successfully') {
      sessionStorage.setItem('technician', true);
